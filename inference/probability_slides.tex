...
\begin{eqnarray*}
{\color{green} P(H, D)} &=& {\color{green}0.01 \times 0.95}\\
{\color{green} P(\neg H, D)} &=& {\color{green} 0.99 \times 0.05}\\
...
\begin{enumerate}
\item Write a list of possible answers to your question.
\item Give a probability to each.
\item Delete the ones you discover are false.
\item Renormalise the remaining probabilities.
\end{enumerate}

It just seems more complicated because we often apply it to more
...
This is why MCMC (next session) is useful when there's more than one
unknown parameter, but it's also related to
...
\begin{eqnarray}
p(D_{\rm new} | D) &=& \int p(D_{\rm new}, \theta | D) \, d\theta\\
&=& \int p(\theta | D)p(D_{\rm new} | \theta, D) \, d\theta\\
&=& \int p(\theta | D)p(D_{\rm new} | \theta) \, d\theta\\
\end{eqnarray}
(usually)
...
\begin{frame}[fragile, t]{Question 5}
